...
def value_of(mdp, s, a, v, gamma):
    ...
    new_v = 0.0
    for s2 in ss:
        new_v += mdp.P(s, a, s2) * (mdp.R(s, a, s2) + gamma * v[s2])
    return new_v
...
def value_iteration(mdp, gamma, epsilon=0.001):
    ...
    const = epsilon * (1 - gamma) / (2 * gamma)
    while True:
        flag = False  # flag to detect if we need to break the loop
        new_v = v.copy()
        for s in mdp.states():
            new_v[s] = max(value_of(mdp, s, a, v, gamma) for a in mdp.applicable_actions(s))
            if flag or abs(new_v[s] - v[s]) >= const:
                flag = True
        v = new_v
        if not flag:
            break
