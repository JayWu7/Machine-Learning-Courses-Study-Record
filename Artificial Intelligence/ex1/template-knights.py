...
class KnightState:
    ...
    def successors(self):
        l_moves = []
        haves = [i[:2] for i in self.occupied]
        for knight in self.occupied:
            for x in [-1, 1, -2, 2]:
                if 0 <= knight[0] + x <= 7:  # otherwise the new x coordinate is illegal, we ignore it
                    y = 1 if abs(x) == 2 else 2
                    if 0 <= knight[1] + y <= 7:
                        new_state = (knight[0] + x, knight[1] + y, not knight[2])
                        if new_state[:2] not in haves:
                            new_states_obj = KnightState([new_state])
                            l_moves.append((str(new_states_obj.occupied), new_states_obj))
                    if 0 <= knight[1] - y <= 7:
                        new_state = (knight[0] + x, knight[1] - y, not knight[2])
                        if new_state[:2] not in haves:
                            new_states_obj = KnightState([new_state])
...
# WB...... BW......
#
breadthFirstSearch(KnightState([(0, 0, True), (1, 2, True)]),
                   lambda state: (state.occupied == [(0, 0, True), (1, 2, True)]))
